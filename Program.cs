    ...
    class Program
    {
        ...
        */
        ...
        public static IList<IList<int>> ThreeSum(int[] nums)
        {
            try
            {
                ...
                Array.Sort(nums);
                for (int sk = 0; sk < nums.Length - 2; sk++)
                {
                    if (sk > 0 && nums[sk] == nums[sk - 1])
                    {
                        continue;
                    }
                    int sp = sk + 1, ss = nums.Length - 1;
                    while (sp < ss)
                    {
                        int cs = nums[sk] + nums[sp] + nums[ss];
                        if (cs == 0)
                        {
                            triplet.Add(new List<int> { nums[sk], nums[sp], nums[ss] });
                            while (sp < ss && nums[sp] == nums[sp + 1]) sp++;
                            while (sp < ss && nums[ss] == nums[ss - 1]) ss--;
                            sp++;
                            ss--;
                        }
                        else if (cs < 0)
        ...
        */
        ...
        public static int MaximumGap(int[] nums)
        {
            try
            {
                ...
                int os = Math.Max(1, (ma_n - mi_n) / (t_n - 1));
                int c_t = (ma_n - mi_n) / os + 1;

                int?[] mi_v = new int?[c_t];
                int?[] ma_v = new int?[c_t];


                foreach (int n in nums)
                {
                    int i_b = (n - mi_n) / os;
                    mi_v[i_b] = mi_v[i_b] == null ? n : Math.Min(mi_v[i_b].Value, n);
                    ma_v[i_b] = ma_v[i_b] == null ? n : Math.Max(ma_v[i_b].Value, n);
                ...
                for (int i = 0; i < c_t; i++)
                {
                    if (!mi_v[i].HasValue) continue;
                    g_m = Math.Max(g_m, mi_v[i].Value - o_m);
                    o_m = ma_v[i].Value;
                }
        ...
        */
        ...
        public static string RemoveOccurrences(string s, string part)
        {
            try
            {
                int i_p = s.IndexOf(part);

                while (i_p != -1)
                {
                    s = s.Remove(i_p, part.Length);
                    i_p = s.IndexOf(part);
                }
                return s;
            }
